...
let registers = amake 255 (false, Bigint.zero)

let ord thechar = int_of_char thechar
type binop_t = bigint -> bigint -> bigint

let rec print_num_string str =
    if String.length str > 69
    then (printf "%s\\\n%!" (String.sub str 0 69);
      print_num_string (String.sub str 69 (String.length str - 69)))
    else printf "%s\n%!" str

let print_number number =
    print_num_string (string_of_bigint number)

let print_stackempty () = printf "dc: stack empty\n%!"

let executereg (thestack: stack_t) (oper: char) (reg: int) =
    try match oper with
        | 'l' ->  let valid, value = aget registers reg in
            if valid
            then push value thestack
            else printf "register '%d' is empty\n%!" reg

        | 's' -> aset registers reg (true, pop thestack)
